        ...
        if (!empty($_POST['add_submit']))
        {
            if (isset($_POST['title'], $_POST['body']) && mb_strlen($_POST['title']) <= 50) // Allow a maximum of 50 characters
            {
                ...
                if ($this->oModel->add($aData))
                    $this->oUtil->sSuccMsg = 'The post has been added!!';
                else
                    $this->oUtil->sErrMsg = 'Oops! An error has occurred! Please try again later.';
            }
            else
            {
        ...
        if (!empty($_POST['edit_submit']))
        {
            if (isset($_POST['title'], $_POST['body']))
            {
                ...
                if ($this->oModel->update($aData))
                    $this->oUtil->sSuccMsg = 'The post has been updated!!';
                else
                    $this->oUtil->sErrMsg = 'Oops! An error has occurred! Please try again later';
            }
            else
